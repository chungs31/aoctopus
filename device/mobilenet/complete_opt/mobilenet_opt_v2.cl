        ...
        for (int jj = 0; jj < Tc; jj++) {
          ...
          for (int di = 0; di < 3; ++di) {
            ...
            for (int dj = 0; dj < 3; ++dj) {
              tmp[jj] += (float)((((((1 - di) <= i) && (i < ((yy_bound+1) - di))) && ((1 - dj) <= (j+jj))) && ((j+jj) < ((yy_bound+1) - dj))) ? input0[(((((((ax1 * yy_bound) + i) + di) * yy_bound) + (j+jj)) + dj) + -(yy_bound+1))] : 0.000000e+00f) * input1[((((ax1 * 3) + di) * 3) + dj)];
            }
          }
        }
        #pragma unroll
        for (int jj = 0; jj < Tc; jj++) {
          T_clip[((((ax1 * yy_bound) + i) * yy_bound) + (j + jj))] = max(min(((tmp[jj] * input2[ax1]) + input3[ax1]), 6.000000e+00f), 0.000000e+00f);
        }
      }
    }
  }
      ...
      for (int j = 0; j < yy_bound; j += Tc) {
        ...
        for (int jj = 0; jj < Tc; jj++) {
          ...
          for (int di = 0; di < 3; ++di) {
            #pragma unroll
            for (int dj = 0; dj < 3; ++dj) {
              tmp[jj] += input0[(((((ax1 * ax1_stride) + (i * i_stride)) + (di * di_stride)) + ((j+jj) * j_stride)) + dj)] * input1[((((ax1 * 3) + di) * 3) + dj)];
            }
          }
        }
        #pragma unroll
...
__kernel void fuse_softmax_kernel0(__global float* restrict tensor, __global float* restrict input0, __global float* restrict tensor1, __global float* restrict tensor2) {
  ...
  for (int k = 0; k < 1000; ++k) {
    maxf = max(maxf, input0[(k)]);
  }
  for (int i1 = 0; i1 < 1000; ++i1) {
    tensor[(i1)] = exp((input0[(i1)] - maxf));
  }
